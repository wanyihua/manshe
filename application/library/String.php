...
class MS_String
{
     ...
     */
    public static function msStrpos($string, $substr, $time = 1) {
        $len = strlen($substr);
        $return  = 0;
        for($i = 0; $i < $time; $i++){
	...
	**/
	public static function iconv_recursive($var, $in_charset = 'UTF-8', $out_charset = 'GBK')
	{
		...
		elseif( is_string($var) )
		{
			$res = iconv($in_charset, $out_charset, $var);
			if( false === $res ){
				return $var;
			}else{
				return $res;
	...
	/**
	 * @brief Convert string or array to requested character encoding use uconv.so
	 *
	 * @param mix $var	variable to be converted, both key and value be converted.
	 * @param string $in_charset	The input charset.
	 * @param string $out_charset	The output charset
	 * @param string $convertKey    Whether or not convert the key 
	 * @return mix	The array with all of the values in it noslashed
	 * @retval The array with all of the values in it noslashed
	 * @note check uconv.so is ok, both key and value be converted.
	 * @author luhaixia
	 * @date 2013/03/20 17:51:09
	**/
	public static function uconv_recursive($var, $in_charset = 'UTF-8', $out_charset = 'GBK', $convertKey = true)
	{
		$in_charset = str_replace('UTF8', 'UTF-8', strtoupper($in_charset));
		$out_charset = str_replace('UTF8', 'UTF-8', strtoupper($out_charset));
		if( is_array($var) )
		{
			$rvar = array();
			foreach( $var as $key => $val )
			{
				if($convertKey === true){
					$rkey = self::uconv_recursive($key, $in_charset, $out_charset, $convertKey);
					$rvar[$rkey] = self::uconv_recursive($val, $in_charset, $out_charset, $convertKey);
				}else{
					$rvar[$key] = self::uconv_recursive($val, $in_charset, $out_charset, $convertKey);
				}
			}
			return $rvar;
		}
		...
		elseif( is_string($var) )
		{
			...
			if ($in_charset == 'GBK' && $out_charset == 'UTF-8') {
				return gbk_to_utf8($var);
			} elseif ($in_charset == 'UTF-8' && $out_charset == 'GBK') {
				return utf8_to_gbk($var, 0x10);
			} else {
				return mb_convert_encoding($var, $out_charset, $in_charset);
			}
		}
		else
		{
